"use client";

import { useEffect } from "react";
import { useRouter } from "next/navigation";
import { useAuth } from "@/lib/auth-context";

export default function LoginPage() {
  const { user, loading, login } = useAuth();
  const router = useRouter();

  useEffect(() => {
    if (!loading && user) router.replace("/dashboard");
  }, [user, loading, router]);

  return (
    <div className="min-h-screen bg-cream flex">
      {/* Left panel — branding */}
      <div className="hidden lg:flex lg:w-1/2 bg-ink flex-col justify-between p-16">
        <div>
          <span className="font-display text-2xl text-cream-card tracking-tight">
            Task<span className="text-accent">Flow</span>
          </span>
        </div>
        <div className="space-y-6">
          <h1 className="font-display text-5xl text-cream-card leading-tight">
            Work together,<br />
            <span className="text-accent">ship faster.</span>
          </h1>
          <p className="text-ink-faint text-lg leading-relaxed max-w-md">
            Assign tasks, track progress, and get notified when things get done — all in one place.
          </p>
        </div>
        <div className="flex gap-8 text-ink-faint text-sm">
          <span>✓ Google sign-in</span>
          <span>✓ Email notifications</span>
          <span>✓ Real-time updates</span>
        </div>
      </div>

      {/* Right panel — auth */}
      <div className="flex-1 flex flex-col items-center justify-center px-8 animate-fade-in">
        <div className="w-full max-w-sm space-y-8">
          {/* Mobile logo */}
          <div className="lg:hidden text-center">
            <span className="font-display text-3xl text-ink tracking-tight">
              Task<span className="text-accent">Flow</span>
            </span>
          </div>

          <div className="space-y-2 text-center lg:text-left">
            <h2 className="text-2xl font-semibold text-ink">Welcome back</h2>
            <p className="text-ink-muted text-sm">Sign in to your workspace</p>
          </div>

          <button
            onClick={login}
            className="w-full flex items-center justify-center gap-3 px-6 py-3.5
              bg-white border border-ink/15 rounded-xl text-ink font-medium text-sm
              hover:border-ink/30 hover:shadow-card transition-all duration-200
              active:scale-[0.99] shadow-card"
          >
            {/* Google icon */}
            <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google
          </button>

          <p className="text-center text-xs text-ink-faint">
            By continuing, you agree to our{" "}
            <span className="underline cursor-pointer">Terms</span> and{" "}
            <span className="underline cursor-pointer">Privacy Policy</span>.
          </p>
        </div>
      </div>
    </div>
  );
}
